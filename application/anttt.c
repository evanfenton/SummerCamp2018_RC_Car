...
Description:
Copies incoming data to the Anttt Rx buffer.
...
/*--------------------------------------------------------------------------------------------------------------------
Function: static bool AntttIsGameOver(void)
...
{
  u8 au8Temp[ANTTT_COMMAND_SIZE] = {0, 0, 0};
  u8 u8Position;
  ...
  if (G_u32BPEngenuicsFlags & _BPENGENUICS_CONNECTED)
  {
    ...
    if (Anttt_u8RxData[ANTTT_COMMAND_ID_OFFSET] == ANTTT_COMMAND_ID_MOVE)
    {
      ...
      {
        // Send NACK response.
        au8Temp[ANTTT_COMMAND_ID_OFFSET] = ANTTT_COMMAND_ID_MOVE_NACK;
      ...
      else
      {
        ...
        // Send response.
        au8Temp[ANTTT_COMMAND_ID_OFFSET] = ANTTT_COMMAND_ID_MOVE_ACK;
        BPEngenuicsSendData(au8Temp, ANTTT_COMMAND_SIZE);
      ...
      /* Regardless, clear the Rx buffer */
      Anttt_reset_rx_buffer();
  ...
  {
    if (WasButtonPressed(u32Button) & !Anttt_bPendingResponse)
    {
...
static void AntttSM_Active(void)
{
  ...
  if (G_u32BPEngenuicsFlags & _BPENGENUICS_SERVICE_ENABLED)
  {
    ...
    {
      if (WasButtonPressed(u8Button) & !Anttt_bPendingResponse)
      {
        ...
        Anttt_bPendingResponse = true;
        return;
      }
      else if (Anttt_bPendingResponse)
      {
        // User may be pressing buttons in this state, Ack the button presses to 
        // disable queuing of the presses as valid game presses.
        ButtonAcknowledge(u8Button);    
    ...
    if (Anttt_u8RxData[ANTTT_COMMAND_ID_OFFSET] == ANTTT_COMMAND_ID_MOVE_ACK)
    {
      Anttt_reset_rx_buffer();
      Anttt_bPendingResponse = false;
